
/**
 * version: 1.1
 * last updated: 2016/03/29     fix: function showId() error
 * 
 * 1.0  2016/03/26 created
 * */

if (window.MM) {
	MM = window.MM;
}else{
	window.MM = {};
}

(function(MM){
		
		MM.mask = {};

		MM.mask.icon = "";

		var ImagePath = "";
		
		try{
			ImagePath = MM.getLocal().split("Scripts")[0];
			ImagePath = ImagePath + "Images/";
	    }catch(err){}
		
		MM.mask.init = function(){

		    var div_mask = document.createElement("div");
		    div_mask.id = "div-mask";
		    div_mask.style.display = "none";
		    div_mask.style.verticalAlign = "middle";
		    div_mask.style.width = "100%";
		    div_mask.style.height = "100%";

		    if (MM.mask.icon == "") {
		        MM.mask.icon = "data:image/gif;base64,";
		    }
			

		    var img = document.createElement("img");
		    img.id = "div-mask-instance-img";
		    img.src = MM.mask.icon;

		    var div_mask_instance = document.createElement("div");
		    div_mask_instance.id = "div-mask-instance";

		    var div_mask_text = document.createElement("div");
			div_mask_text.id = "div-mask-text";
			div_mask_text.innerText = "";
			
		    div_mask.appendChild(div_mask_instance);
			div_mask.appendChild(img);
			div_mask.appendChild(div_mask_text);

			var tags = document.getElementsByTagName("body");
			if (tags.length <= 0) return;
			var body = tags[0];
			body.appendChild(div_mask);
			

			var mask = document.getElementById("div-mask-instance");
			mask.className = "div-mask-instance";
			mask.style.zIndex = "999999999";
			mask.style.position = "absolute";
			mask.style.top = "0px";
			mask.style.left = "0px";
			mask.style.width = document.body.scrollWidth + "%";
			mask.style.height = document.body.scrollHeight + "%";
			mask.style.backgroundColor = "#B1B1B1";
			mask.style.color = "#ffffff";
			mask.style.fontSize = "14px";
			mask.style.textAlign = "center";
			mask.style.verticalAlign = "middle";
			mask.style.opacity = ".5";

			mask.style.filter = "alpha(opacity=50)";
			
			try{
				$("#div-mask-instance").css("-moz-opacity",".5");
				$("#div-mask-instance").css("-khtml-opacity",".5");
			}catch(err){}
			
			var img = document.getElementById("div-mask-instance-img");
			img.style.zIndex = "999999999";
			img.style.margin = "0";
			img.style.position = "absolute";
			img.style.top = "50%";
			img.style.left = "50%";
			img.style.marginRight = "-50%";
			img.style.transform = "translate(-50%, -50%)";

			var text = document.getElementById("div-mask-text");
			text.style.zIndex = "999999999";
			text.style.margin = "0";
			text.style.position = "absolute";
			text.style.top = "53%";
			text.style.left = "50%";
			text.style.marginRight = "0%";
			text.style.transform = "translate(-50%, -50%)";
			text.style.color = "#fff";
			text.style.fontSize = "15px";
			text.style.fontFamily = "Microsoft JhengHei;";
		}

		
		MM.mask.show = function () {

			
		    if (document.getElementById("div-mask") == null) {
		        MM.mask.init();
		    } else {

		        if (MM.mask.icon != "") {
		            document.getElementById("div-mask-instance-img").src = MM.mask.icon;
		        }

		        var mask = document.getElementById("div-mask-instance");
		        mask.style.width = document.body.scrollWidth + "%";
		        mask.style.height = document.body.scrollHeight + "%";
		    }

			document.getElementById("div-mask").style.display = "";
			

		}


		MM.mask.showInText = function(text){

		    if (document.getElementById("div-mask") == null) {
		        MM.mask.init();
		    } else {

		        if (MM.mask.icon != "") {
		            document.getElementById("div-mask-instance-img").src = MM.mask.icon;
		        }

		        var mask = document.getElementById("div-mask-instance");
		        mask.style.width = document.body.scrollWidth + "%";
		        mask.style.height = document.body.scrollHeight + "%";
		    }
			
			if(text == null) text = "Loading";

			document.getElementById("div-mask-text").innerText = text;
			document.getElementById("div-mask").style.display = "";

		    //position
			var img = document.getElementById("div-mask-instance-img");
			var text = document.getElementById("div-mask-text");
			text.style.top = (img.offsetTop / 2 + img.offsetHeight + 20) + "px";
		}
		MM.mask.hide = function () {
		    setTimeout(function () {
		        if(document.getElementById("div-mask"))
		            document.getElementById("div-mask").style.display = "none";
		    }, 200);

		}
		MM.mask.showId = function (id) {

		    if (id == null || id == "") return;

		    var t = document.getElementById(id);

		    t.readOnly = true;
		    t.style.backgroundImage = "url('data:image/gif;base64,')";
		    t.style.backgroundColor = "#e2e2e2";
		    t.style.backgroundRepeat = "no-repeat";
		    t.style.backgroundPositionX = "right";

		}
		MM.mask.hideId = function (id) {

		    if (id == null || id == "") return;

		    var t = document.getElementById(id);

		    t.readOnly = false;
		    t.style.backgroundImage = "";
		    t.style.backgroundColor = "";
		    t.style.backgroundRepeat = "";
		    t.style.backgroundPositionX = "";
		}


})(MM);