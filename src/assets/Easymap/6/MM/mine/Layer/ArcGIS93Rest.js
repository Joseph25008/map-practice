...
OpenLayers.Layer.ArcGIS93Rest = OpenLayers.Class(OpenLayers.Layer.Grid, {
    ...
    //Ｅａｓｙｍａｐ
    mm: null,
    mm_projection: "EPSG:4326",//目標的坐標系統
    /**
    ...
    initialize: function(name, url, params, options) {
        ...
        //rextseng 
        this.mm = options.mm;
        this.mm_projection = options.mm_projection;
    ...
    getURL: function (bounds) {
        ...
        bounds = bounds.transform(proj3857, newproj);
        
        var sr = '4326';
        switch(this.mm_projection.toUpperCase()){
            case "EPSG:4326":
                sr = '4326';
                break;
            case "EPSG:3826"://97
                sr = '3826';
                map.mm.coordTrans.from(bounds.left, bounds.top, "WGS84");
                var p = map.mm.coordTrans.toTM2_97();
                bounds.left = p.x;
                bounds.top = p.y;

                map.mm.coordTrans.from(bounds.right, bounds.bottom, "WGS84");
                var p = map.mm.coordTrans.toTM2_97();
                bounds.right = p.x;
                bounds.bottom = p.y;
                break;
            case "EPSG:3827"://67
                sr = '3827';
                map.mm.coordTrans.from(bounds.left, bounds.top, "WGS84");
                var p = map.mm.coordTrans.toTM2_67();
                bounds.left = p.x;
                bounds.top = p.y;
